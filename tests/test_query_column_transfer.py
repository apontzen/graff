...
def test_column_transfer():
    results = n.query("person").with_property("net_worth").follow("owns").with_property("price").all()
...
def test_column_all_property_transfer():
    results = n.query("person").with_property().follow("owns").with_property("price").all()
    assert len(results[0]) == 3
...
def test_column_backreference_in_filter():
    q1 = n.query("person").with_property("net_worth", "name")
